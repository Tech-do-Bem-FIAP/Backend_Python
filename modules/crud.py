...
from modules import api_externa, storage, utils
...
def cadastrar_paciente(usuario_logado: dict) -> dict | None:
    ...
    # Busca de endereço via ViaCEP (API pública)
    print("\n  Endereço (busca automática via ViaCEP):")
    endereco = api_externa.coletar_endereco_interativo()
...
def editar_paciente(usuario_logado: dict) -> None:
    ...
    cep_atual = alvo.get("cep") or "não informado"
    if utils.confirmar(f"  Atualizar endereço? (CEP atual: {cep_atual}) (S/N): "):
        endereco = api_externa.coletar_endereco_interativo()
        if endereco.get("cep"):
            alvo["cep"] = endereco["cep"]
            alvo["logradouro"] = endereco["logradouro"]
            alvo["bairro"] = endereco["bairro"]
            alvo["cidade"] = endereco["cidade"]
            alvo["uf"] = endereco["uf"]
